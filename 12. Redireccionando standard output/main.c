...
int main(int argc, char const *argv[]) {
    ...
    if (pid == 0) {
        ...
        int file2 = dup2(file, STDOUT_FILENO); //toma 2 parámetros: el primero es el file descriptor que queremos clonar, y el segundo es el valor que queremos que el nuevo file descriptor tenga. Además, cierra el filedescriptor especificado en el segundo parámetro. Ahora tenemos el file descriptor de 3, y el de valor 1 que reemplazó a standard output por pingResults.txt. Ahora cuando el programa ping quiera imprimir algo por pantalla (como los resultados del ping), éstos irán al archivo .txt en lugar de mostrarse por pantalla, dado que "redireccionamos" standard output al archivo especificado por el filedescriptor file.
